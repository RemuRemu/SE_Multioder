    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (username != null) {
                ...
                System.out.println("charge success: " + (charge.getStatus().equals(ChargeStatus.Successful)));
                System.out.println("failed message: " + charge.getFailureMessage());

                if (!charge.getStatus().equals(ChargeStatus.Successful)) {
                    out.print("<script>alert('" + charge.getFailureMessage() + "');history.back(-1)</script>");
                    return;
                } else {
